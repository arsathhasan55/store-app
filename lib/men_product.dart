...
class MenProductListingPage extends StatefulWidget {
  const MenProductListingPage({super.key});

  @override
  State<MenProductListingPage> createState() => _MenProductListingPageState();
}

class _MenProductListingPageState extends State<MenProductListingPage> {
  final List<String> _subCategories = ['All', 'Shirt', 'T-Shirt', 'Pants', 'Caps'];
  String _selectedCategory = 'All';
  String _searchQuery = '';

  String _resolveImagePath(String? path) {
    if (path == null || path.isEmpty) return 'lib/assets/images/men category/shirts/shirt.png';
            ...
            children: [
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: const Row(
                        ...
                        children: [
                          Icon(Icons.tune_rounded, color: AppTheme.primaryColor, size: 14),
                          SizedBox(width: 4),
                          Text('Filter', style: TextStyle(color: AppTheme.primaryColor, fontWeight: FontWeight.bold, fontSize: 13)),
                        ],
                      ),
                    ),
                  ],
              ...
              Expanded(
                child: StreamBuilder<List<ProductModel>>(
                  stream: context.read<ProductService>().getProductsByCategory('men'),
                  builder: (context, snapshot) {
                    ...
                    if (_selectedCategory != 'All') {
                      products = products.where((p) => p.subCategory.toLowerCase() == _selectedCategory.toLowerCase()).toList();
                    }
                    ...
                    if (products.isEmpty) {
                      return Center(
                        child: Column(
                          ...
                          children: [
                            const Icon(Icons.person_outline_rounded, size: 60, color: Colors.white24),
                            const SizedBox(height: 16),
                            Text(
                              _searchQuery.isNotEmpty ? 'No results for "$_searchQuery"' : 'No items in this category yet.',
